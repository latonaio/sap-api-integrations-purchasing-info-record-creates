...
func main() {
	...
	inputSDC := fr.ReadSDC("./Inputs/SDC_Purchasing_Info_Record_General_Purchasing_Organization_Plant_sample.json")
	sap_api_time_value_converter.ChangeTimeFormatToSAPFormatStruct(&inputSDC)

	accepter := getAccepter(inputSDC)
	generalPurchasingOrganizationPlant := inputSDC.ConvertToGeneralPurchasingOrganizationPlant()

	caller.AsyncPostPurchasingInfoRecord(
		generalPurchasingOrganizationPlant,
		accepter,
	)
}
...
func getAccepter(sdc sap_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"GeneralPurgInfoRecdOrgPlantData",
		}
	}
	return accepter
}
